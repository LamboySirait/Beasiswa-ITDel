...
<body>
    ...
    <div class="w-3/4 m-auto flex flex-col items-end">
        @csrf
        <a href="{{route('create-data-beasiswa', $year)}}" type="submit" class="rounded-md px-3 py-2 text-white font-bold hover:shadow-slate-400 hover:shadow-md hover:bg-green-700 focus:ring-4 focus:outline-none focus:ring-green-500" style="background-color: #0D285F;">Tambah Data</a>
    </div>
    @endif
    @endif
    <table class="border-collapse border border-slate-400 mx-auto mt-10 mb-5 shadow-2xl">
        <thead style="background-color: #0D285F;" class="text-white">
        ...
        <tr class="border text-center ">
            ...
            <td class="border border-slate-400"> <!-- Tambah baris ini -->
                <a href="{{route('edit-data-beasiswa', [$item->id, $year])}}" class="px-4 py-1 rounded-md text-white font-bold uppercase" data-toggle="tooltip" title='Edit' style="background-color: #0D285F;">Edit</a>
            </td>
            @endif
            @endif
